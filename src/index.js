...
function Process(keys) {
  ...
  (this.splitObjectIntoTwoArrays = obj => {
        ...
        return new Promise(async (resolve, reject) => {
          ...
          try {
            ...
            for (let x = 0; x < arrayOfValues.length; x++) {
              let eValue = this.encode.encodeValue(
                arrayOfValues[x]
              );
  ...
  this.init = async () => {
    let { encodedValues, encryptionKey } =
      await this.encode.init(this.keys());
  ...
  this.decodeValues = async (values, encryptionKey) => {
  ...
  this.decodeValue = (v, encryptionKey) => {
    return new Promise(async (resolve, reject) => {
  ...
  this.getCipherTextFromUIntArray = () => {};
}

const fdd = new Process(keys);
fdd.init();
